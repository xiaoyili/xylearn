...
from xylearn.utils.theano_util import toFloatX, toConstantX
...
def theano_norms(W):
    ...
    theano variables
    """
    return T.sqrt(toFloatX(1e-8) + T.sqr(W).sum(axis=0))
...
def symGivens2(a, b):
    ...
    considerably ..
    """
    c_branch1 = T.switch(T.eq(a, toConstantX(0)),
                         toConstantX(1),
                         T.sgn(a))
    c_branch21 = (a / b) * T.sgn(b) / \
                 T.sqrt(toConstantX(1) + (a / b) ** 2)
    c_branch22 = T.sgn(a) / T.sqrt(toConstantX(1) + (b / a) ** 2)

    c_branch2 = T.switch(T.eq(a, toConstantX(0)),
                         toConstantX(0),
                         T.switch(T.gt(abs(b), abs(a)),
                                  c_branch21,
                                  c_branch22))
    c = T.switch(T.eq(b, toConstantX(0)),
                 c_branch1,
                 c_branch2)

    s_branch1 = T.sgn(b) / T.sqrt(toConstantX(1) + (a / b) ** 2)
    s_branch2 = (b / a) * T.sgn(a) / T.sqrt(toConstantX(1) + (b / a) ** 2)
    s = T.switch(T.eq(b, toConstantX(0)),
                 toConstantX(0),
                 T.switch(T.eq(a, toConstantX(0)),
                          T.sgn(b),
                          T.switch(T.gt(abs(b), abs(a)),
                                   s_branch1,
                                   s_branch2)))

    d_branch1 = b / (T.sgn(b) / T.sqrt(toConstantX(1) + (a / b) ** 2))
    d_branch2 = a / (T.sgn(a) / T.sqrt(toConstantX(1) + (b / a) ** 2))
    d = T.switch(T.eq(b, toConstantX(0)),
                 abs(a),
                 T.switch(T.eq(a, toConstantX(0)),
                          abs(b),
                          T.switch(T.gt(abs(b), abs(a)),
