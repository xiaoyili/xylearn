...
updates_generate : dictionary of Theano variable -> Theano variable
  ...
  generation function.'''
    ...
    if n_out:
        ...
        ft_params = Wy, by, bh, W

        y = T.ivector('y')
        # y = T.matrix('y')

    params = W, bv, bh, Wuh, Wvu, Wuu, bu  # learned parameters as shared
    # variables
    ...
    def recurrence(v_t, u_tm1):
        ...
        if generate:
            ...
            #                                bh_t, k=25)
            v_t, _, _, updates = build_ae(T.zeros((n_visible,)), W, bv,
                                          bh_t)

        u_t = T.nnet.sigmoid(bu + T.dot(v_t, Wvu) + T.dot(u_tm1, Wuu))

        return ([v_t, u_t], updates) if generate else [u_t, bh_t]

    # For training, the deterministic recurrence is used to compute all the
    # {bv_t, bh_t, 1 <= t <= T} given v. Conditional RBMs can then be trained
    # in batches using those parameters.
    (u_t, bh_t), updates_train = theano.scan(
        lambda v_t, u_tm1, *_: recurrence(v_t, u_tm1),
        sequences=v, outputs_info=[u0, None], non_sequences=params)

    v_sample, cost, monitor, updates_rbm = build_ae(v, W, bv, bh_t[:])
    ...
    if n_out is None:
        ...
        h_mean = T.nnet.sigmoid(T.dot(v, W) + bh)

        p_y_given_h = T.nnet.softmax(T.dot(h_mean, Wy) + by)
        ...
        return (v, v_sample, cost, monitor, params, updates_train,
                v_t, updates_generate,
                y, ft_cost, ft_params, ft_errors, y_pred)
...
def test_rnnrbm(batch_size=100, num_epochs=10):
